...
const calculateCost = (fees, inTime, outTime, totalTime) => {
  ...
  if (!inTime && !outTime)
    return totalTime > baseTime
      ? baseCost + Math.ceil((totalTime - baseTime) / unitTime) * unitCost
      : baseCost;

  const timeDiff = getTime(outTime) - getTime(inTime);
  return (
    (timeDiff + totalTime >= baseTime
      ? Math.ceil((timeDiff + totalTime - baseTime) / unitTime) * unitCost
      : 0) + baseCost
  );
};
...
const solution = (fees, records) => {
  ...
  records.forEach((record) => {
    ...
    if (!infos[carNumber]) {
      ...
      infos[carNumber] = {
        ...
        totalTime:
          memo.totalTime +
          (status === "OUT" ? getTime(time) - getTime(memo.time) : 0),
      };
    }
  });
  ...
  const answer = Object.entries(infos)
    .sort((a, b) => a[0] - b[0])
