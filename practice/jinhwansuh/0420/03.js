const getShuttleTable = (n, t) => {
  const array = [];
  let time = 60 * 9;
  for (let i = 0; i < n; i++) {
    array.push({ time, people: 0 });
    time += t;
  }
  return array;
};

const stringTimetoMinute = (stringTime) => {
  const [hour, minute] = stringTime.split(':');
  return +hour * 60 + +minute;
};
const minuteToStringTime = (minuteTime) => {
  const hour = parseInt(minuteTime / 60, 10);
  const minute = minuteTime % 60;
  const time =
    String(hour).padStart(2, '0') + ':' + String(minute).padStart(2, '0');
  return time === '24:00' ? '00:00' : time;
};

function solution(n, t, m, timetable) {
  const minuteTable = timetable.map((time) => stringTimetoMinute(time));
  minuteTable.sort((a, b) => a - b);

  const shuttleTimeTable = getShuttleTable(n, t);
  // console.log(shuttleTimeTable);

  if (timetable.length < m) {
    return minuteToStringTime(
      shuttleTimeTable[shuttleTimeTable.length - 1].time
    );
  }

  for (const time of minuteTable) {
    for (let i = 0; i < shuttleTimeTable.length; i++) {
      if (
        shuttleTimeTable[i].time >= time &&
        i === shuttleTimeTable.length - 1 &&
        shuttleTimeTable[i].people + 1 === m
      ) {
        return minuteToStringTime(time - 1);
      }
      if (
        shuttleTimeTable[i].time >= time &&
        shuttleTimeTable[i].people + 1 <= m
      ) {
        shuttleTimeTable[i].people = shuttleTimeTable[i].people + 1;
        break;
      }
    }
  }

  const lastBus = shuttleTimeTable[shuttleTimeTable.length - 1];
  if (lastBus.people < m) {
    return minuteToStringTime(lastBus.time);
  }
}
